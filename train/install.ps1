...
Write-Host ""
Write-Host "[5/6] Creating command shortcuts..." -ForegroundColor Yellow
...
if (Test-Path $batSource) {
    if (-not (Test-Path $batDest)) {
        New-Item -ItemType Directory -Path $batDest -Force | Out-Null
    }
    ...
    Copy-Item -Path "$batSource\hypertrain.bat" -Destination $currentDir -Force
    Write-Host "✅ hypertrain.bat created in current directory" -ForegroundColor Green
} else {
    Write-Host "⚠️  Batch files not found, creating fallback..." -ForegroundColor Yellow
    $fallbackBat = "@echo off`npython -m hyperneural_turbo %*"
    $fallbackBat | Out-File "$currentDir\hypertrain.bat" -Encoding ASCII
    Write-Host "✅ Fallback hypertrain.bat created" -ForegroundColor Green
}
