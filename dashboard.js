...
async function loadModels() {
    ...
    if (models.length === 0) {
        ...
        grid.innerHTML = models.map(m => `
            <div class="relative group bg-[#0a0a0a]/80 backdrop-blur-xl border border-[#1f1f1f] rounded-2xl p-6 overflow-hidden transition-all duration-500 hover:border-cyan-500/30 hover:shadow-[0_8px_30px_rgba(6,182,212,0.1)] flex flex-col h-full">
                ...
                <div class="relative z-10 flex flex-col h-full">
                    ...
                    <div class="mt-auto flex gap-3 pt-4 border-t border-[#1f1f1f]/50">
                        <button class="flex-1 bg-white/[0.03] border border-[#1f1f1f] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-white/[0.08] hover:border-white/20 transition-all">Details</button>
                        <button class="bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-4 py-2 rounded-lg text-sm font-medium hover:bg-cyan-500 hover:text-black transition-all">Logs</button>
                    </div>
                </div>
            </div>
        `).reverse().join('');
