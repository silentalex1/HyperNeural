...
<body class="bg-[#030303] text-gray-200 font-sans min-h-screen flex">
    ...
    <div class="ml-64 flex-1 min-h-screen">
        ...
        <div id="section-overview" class="p-8">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="stat-card">
                    <p class="text-xs text-gray-500 uppercase tracking-widest mb-2">Total Requests</p>
                    <p class="text-3xl font-black text-white" id="stat-requests">0</p>
                </div>
                <div class="stat-card">
                    <p class="text-xs text-gray-500 uppercase tracking-widest mb-2">Avg Latency</p>
                    <p class="text-3xl font-black text-white" id="stat-latency">—</p>
                </div>
                <div class="stat-card">
                    <p class="text-xs text-gray-500 uppercase tracking-widest mb-2">Files Hosted</p>
                    <p class="text-3xl font-black text-cyan-400" id="stat-files">0</p>
                </div>
            </div>
    ...
    <div id="deploy-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-[#070707] border border-[#1f1f1f] rounded-2xl p-8 w-full max-w-lg">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-lg font-bold text-white">Deploy New Model</h2>
                    <p class="text-xs text-gray-500 mt-0.5">Name your model and optionally upload a GGUF file to host.</p>
                </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input id="model-name-input" type="text" placeholder="example: SuperBot" class="w-full px-4 py-3 bg-[#030303] border border-[#1a1a1a] rounded-xl text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-cyan-500/50 transition-colors">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 uppercase tracking-wider mb-2">Base Model <span class="text-gray-600 normal-case">(Ollama model name, e.g. llama3.2)</span></label>
                    <input id="ollama-model-input" type="text" placeholder="llama3.2" class="w-full px-4 py-3 bg-[#030303] border border-[#1a1a1a] rounded-xl text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-cyan-500/50 transition-colors">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 uppercase tracking-wider mb-2">Upload Model Files <span class="text-gray-600 normal-case">(optional · .gguf, .safetensors, .bin, .jsonl)</span></label>
                    <div id="drop-zone" class="relative border-2 border-dashed border-[#1f1f1f] rounded-xl p-6 text-center hover:border-cyan-500/40 transition-colors">
                        <input type="file" id="file-input" class="hidden" multiple onchange="handleFilesSelect(this.files)">
                        <input type="file" id="folder-input" class="hidden" webkitdirectory directory multiple onchange="handleFilesSelect(this.files)">
                        <svg class="w-8 h-8 text-gray-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                        <p class="text-sm text-gray-500 mb-3" id="drop-label">Drop files or a project folder here</p>
                        <div class="flex items-center justify-center gap-2">
                            <button type="button" onclick="document.getElementById('file-input').click()" class="px-3 py-1.5 text-xs font-bold text-gray-300 bg-white/[0.04] border border-white/10 rounded-lg hover:bg-white/[0.08] transition-all">Select Files</button>
                            <button type="button" onclick="document.getElementById('folder-input').click()" class="px-3 py-1.5 text-xs font-bold text-cyan-400 bg-cyan-500/10 border border-cyan-500/20 rounded-lg hover:bg-cyan-500/20 transition-all">Select Project Folder</button>
                        </div>
                        <p class="text-xs text-gray-600 mt-3">Select multiple individual files, or one folder containing your whole project</p>
                    </div>
                    <div id="file-list" class="mt-3 space-y-1.5 max-h-40 overflow-y-auto"></div>
                </div>
