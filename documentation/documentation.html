...
username: alice
name: SupportBot
ollamaModel: llama3.2          (optional — base Ollama model)
modelFiles: [file, file, ...]  (optional — one or many files, or a full folder)
relativePaths: ["a.gguf"]      (optional — JSON array of relative paths for folder uploads)

Response: { "success": true, "model": { "id": "...", "slug": "supportbot", "status": "Running", "files": [...], ... } }</pre>
            <p>Uploading a <code>.gguf</code> file automatically registers it as a live model in Ollama. <code>.safetensors</code> and <code>.bin</code> files are stored but require GGUF conversion before they can be served. <code>.jsonl</code> files are loaded into the model's knowledge base automatically.</p>
            <h3>List Models</h3>
...
Response: { "success": true, "response": "Our refund policy allows..." }</pre>
            <p>Requires Ollama running on the server with the model pulled. The response uses the model's active version system instructions, knowledge base, and smart training options.</p>
        </div>
