...
async function sendChat() {
    ...
    if (!prompt) return;
    if (modelData.status === 'Initializing') {
        const messages = document.getElementById('chat-messages');
        const notice = document.createElement('div');
        notice.className = 'flex justify-start';
        notice.innerHTML = `<div class="chat-msg model" style="color:#facc15">Your model is still registering its weights. Try again in a moment.</div>`;
...
function startStatusPolling() {
    ...
    if (modelData.status !== 'Initializing') return;
    pollTimer = setInterval(async () => {
        ...
        renderEngineBanner();
        renderFiles();
        if (modelData.status !== 'Initializing') clearInterval(pollTimer);
    }, 4000);
}

init().then(startStatusPolling);
