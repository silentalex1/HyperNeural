...
function handleFileSelect(file) {
    ...
    document.getElementById('drop-label').innerHTML = `<span class="text-cyan-400 font-bold">${file.name}</span> <span class="text-gray-500">(${size})</span>`;
    document.getElementById('drop-zone').classList.add('border-cyan-500/40');
}
...
function closeDeployModal() {
    ...
    document.getElementById('drop-label').innerHTML = 'Drop your model file here or <span class="text-cyan-400">browse</span>';
    document.getElementById('drop-zone').classList.remove('border-cyan-500/40');
...
async function deployModel() {
    ...
    btn.textContent = selectedFile ? 'Uploading...' : 'Deploying...';
    btn.disabled = true;
    ...
    if (selectedFile) formData.append('modelFile', selectedFile);
