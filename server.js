...
const DEFAULT_MODEL = MODEL_ALIASES.sonnet;
...
function resolveModel(baseModel) {
  const key = String(baseModel || "").trim().toLowerCase();
  return MODEL_ALIASES[key] || DEFAULT_MODEL;
}
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const base = (baseModel && baseModel.trim().toLowerCase()) || "sonnet";
