...
const upload = multer({ storage, limits: { fileSize: 10 * 1024 * 1024 * 1024, files: 5000 } });
...
function updateModelById(modelId, updater) {
    const users = getUsers();
    for (const username in users) {
        const model = users[username].models.find(m => m.id === modelId);
        if (model) {
            updater(model);
            saveUsers(users);
            return;
...
function ollamaSafeTag(modelId) {
    return `hn-${modelId}`.toLowerCase().replace(/[^a-z0-9._-]/g, '');
}
...
function moveIntoStructure(files, relativePaths, modelId) {
    const targetRoot = path.join(uploadDir, modelId);
    ...
    files.forEach((file, i) => {
        ...
        const destPath = path.join(targetRoot, cleanRel);
        ...
        results.push({
            ...
            size: file.size,
            ext: path.extname(cleanRel).toLowerCase(),
...
function findWeightFile(filesList) {
    const gguf = filesList.find(f => f.ext === '.gguf');
    if (gguf) return { file: gguf, type: 'gguf' };
    const safetensors = filesList.find(f => f.ext === '.safetensors');
    if (safetensors) return { file: safetensors, type: 'safetensors' };
    const bin = filesList.find(f => f.ext === '.bin');
    if (bin) return { file: bin, type: 'bin' };
    return null;
}

async function registerOllamaModel(modelId, ggufAbsolutePath) {
    const tag = ollamaSafeTag(modelId);
    const modelfileDir = path.join(uploadDir, modelId);
    const modelfilePath = path.join(modelfileDir, 'Modelfile');
    fs.writeFileSync(modelfilePath, `FROM ${ggufAbsolutePath}\n`);

    try {
        await execAsync(`ollama create ${tag} -f "${modelfilePath}"`, { maxBuffer: 1024 * 1024 * 20, timeout: 1000 * 60 * 30 });
        return { success: true, tag };
    } catch (err) {
        return { success: false, error: err.message?.slice(0, 500) || 'Registration failed.' };
    }
}
...
async function runInference(systemInstructions, knowledgeText, smartTraining, prompt, ollamaModel) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Ollama returned ${response.status}: ${txt.slice(0, 300)}`);
    }
...
app.post('/api/models/create', upload.array('modelFiles', 500), async (req, res) => {
    ...
    if (req.files && req.files.length > 0) {
        storedFiles = moveIntoStructure(req.files, relativePaths, modelId);
    }
    ...
    const weight = findWeightFile(storedFiles);
    const jsonlFiles = storedFiles.filter(f => f.ext === '.jsonl');

    if (jsonlFiles.length > 0) {
        let combinedText = '';
        for (const jf of jsonlFiles) {
            try {
                const raw = fs.readFileSync(jf.absolutePath, 'utf8');
                const lines = raw.split('\n').filter(Boolean).slice(0, 200);
                combinedText += `\n\n${jf.name}:\n` + lines.join('\n');
            } catch (_) {}
        }
        newModel.versions[0].knowledgeText = combinedText.slice(0, 40000);
    }

    if (weight && weight.type === 'gguf') {
        newModel.status = 'Initializing';
        ...
        registerOllamaModel(modelId, weight.file.absolutePath).then(result => {
            updateModelById(modelId, m => {
                if (result.success) {
                    m.registeredOllamaModel = result.tag;
                    m.status = 'Running';
                } else {
                    m.status = 'Failed';
                    m.registrationError = result.error;
                }
            });
        });
        return;
    } else if (weight && (weight.type === 'safetensors' || weight.type === 'bin')) {
        newModel.status = 'Needs Conversion';
        newModel.registrationError = `Found a .${weight.type} weight file. HyperNeural currently runs GGUF format models through Ollama — convert your model with llama.cpp's convert script and re-upload the .gguf file. Your model will keep working on its base model in the meantime.`;
    } else {
...
app.post('/api/models/:slug/generate', async (req, res) => {
    ...
    if (!prompt) return res.json({ success: false, status: 'bad_request', error: 'Prompt is required.' });
    ...
    if (!model) return res.json({ success: false, status: 'not_found', error: 'Model not found.' });

    const version = model.versions.find(v => v.tag === model.currentVersion);
    const effectiveModel = model.registeredOllamaModel || model.ollamaModel || DEFAULT_MODEL;
    ...
    try {
        ...
        res.json({
            ...
            latency,
            model: model.name,
            usingFallback: model.status === 'Needs Conversion'
        });
    } catch (err) {
        res.json({
            success: false,
            status: 'engine_offline',
            error: `Could not reach the inference engine. Make sure Ollama is running with model "${effectiveModel}" pulled.`
        });
    }
});
...
app.post('/api/models/:slug/delete', async (req, res) => {
    const { code } = req.body;
    const slug = req.params.slug;
    const users = getUsers();
    let deleted = false;
    ...
    for (const username in users) {
        ...
        if (index !== -1) {
            ...
            const targetModel = users[username].models[index];
            const modelDir = path.join(uploadDir, targetModel.id);
            if (fs.existsSync(modelDir)) fs.rmSync(modelDir, { recursive: true, force: true });
            if (targetModel.registeredOllamaModel) {
                try { await execAsync(`ollama rm ${targetModel.registeredOllamaModel}`); } catch (_) {}
            }
            users[username].models.splice(index, 1);
            delete users[username].deleteCode;
            delete users[username].deleteCodeExpiry;
            logActivity(users, username, 'model_deleted', `Deleted model "${targetModel.name}"`);
            deleted = true;
            uName = username;
            break;
        }
