...
const upload = multer({ storage, limits: { fileSize: 2 * 1024 * 1024 * 1024, files: 500 } });
...
app.post('/api/models/create', upload.array('modelFiles', 500), async (req, res) => {
    ...
    if (weight && weight.type === 'gguf') {
        const result = await registerOllamaModel(modelId, weight.file.absolutePath);
        if (result.success) {
            newModel.registeredOllamaModel = result.tag;
            newModel.status = 'Running';
        } else {
            newModel.status = 'Failed';
            newModel.registrationError = result.error;
        }
