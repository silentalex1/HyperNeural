...
const MAX_UPLOAD_FILE_BYTES = Number(process.env.MAX_UPLOAD_FILE_BYTES) || 100 * 1024 * 1024 * 1024;
const MIN_FREE_DISK_BYTES = Number(process.env.MIN_FREE_DISK_BYTES) || 2 * 1024 * 1024 * 1024;
const OLLAMA_QUICK_TIMEOUT_MS = 5000;
const OLLAMA_INFERENCE_TIMEOUT_MS = Number(process.env.OLLAMA_INFERENCE_TIMEOUT_MS) || 10 * 60 * 1000;
...
async function deployToOllamaAndUpdateStatus(modelId, baseModel, systemPrompt, knowledgeBase, params, examples) {
  const models = readJSON(MODELS_FILE);
  const model = models[modelId];
  if (!model) return;
  try {
    await registerOllamaModel(model.ollamaModelName, baseModel, systemPrompt, knowledgeBase, params, examples);
...
function ollamaRequest(method, urlPath, body, timeoutMs) {
  return new Promise((resolve, reject) => {
    const limit = timeoutMs || OLLAMA_QUICK_TIMEOUT_MS;
    let settled = false;
    ...
    const guard = setTimeout(() => {
      req.destroy();
      finish(reject, new Error("Ollama request timed out"));
    }, limit);
      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 400) {
            finish(reject, new Error(`Ollama responded with status ${res.statusCode}: ${raw}`));
            return;
          }
...
async function isOllamaAvailable() {
  try {
    await ollamaRequest("GET", "/api/tags", null, OLLAMA_QUICK_TIMEOUT_MS);
    return true;
  } catch (err) {
    return false;
  }
...
async function requireOllamaReady(req, res, next) {
  const ready = await isOllamaAvailable();
  if (!ready) {
    return drainAndRespond(req, res, 503, {
      error:
        "The AI inference engine (Ollama) is not reachable right now, so models cannot be deployed or hosted. Make sure Ollama is installed and running, then try again."
    });
  }
  next();
}
...
function requireDiskSpace(req, res, next) {
  ...
  if (free !== null && contentLength > 0 && contentLength > free - MIN_FREE_DISK_BYTES) {
    return drainAndRespond(req, res, 413, {
      error: "Not enough free disk space on the server to store this upload. Free up space or upload a smaller model."
    });
  }
  next();
}
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const initialStatus = hasOnlySafetensors ? "needs_conversion" : "deploying";
      ...
      if (hasGguf) {
        ...
        deployToOllamaAndUpdateStatus(id, ggufFile.path, "", "", model.params, model.examples).catch(() => {});
      } else if (hasOnlySafetensors) {
        registerOllamaModel(model.ollamaModelName, base, "", "", model.params, model.examples).catch(() => {});
      } else {
        deployToOllamaAndUpdateStatus(id, base, "", "", model.params, model.examples).catch(() => {});
      }
...
app.post("/api/models/:idOrSlug/generate", async (req, res) => {
  try {
    ...
    if (!model) return res.status(404).json({ error: "Model not found." });
    if (model.status === "converting" || model.status === "deploying") {
      return res.status(409).json({ error: "This model is still being prepared and isn't ready to answer requests yet." });
    }
    ...
    if (err.code === "ECONNREFUSED") {
      return res.status(503).json({ error: "The AI inference engine (Ollama) is not reachable right now." });
    }
...
app.use((err, req, res, next) => {
  ...
  if (err && err.code === "LIMIT_FILE_SIZE") {
    const limitGb = Math.round(MAX_UPLOAD_FILE_BYTES / (1024 * 1024 * 1024));
    return res.status(413).json({ error: `One of your files exceeds the ${limitGb}GB upload limit.` });
  }
  console.error(err);
