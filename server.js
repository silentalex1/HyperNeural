...
function extractKnowledgeBase(files) {
  let combined = "";
  const maxFilesToProcess = 20;
  const processedFiles = 0;
  
  for (const f of files.slice(0, maxFilesToProcess)) {
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedExtensions = ['.bin', '.gguf', '.safetensors', '.pt', '.pth', '.onnx', '.ckpt', '.json', '.txt', '.md', '.jinja', '.jinja2', '.model', '.vocab', '.merges', '.yaml', '.yml', '.msgpack', '.h5', '.npz', '.tflite', '.mlmodel', '.pb', '.index'];
...
app.get("/connect/account/data", (req, res) => {
  res.sendFile(path.join(__dirname, "connect", "account", "data"));
});

app.post("/api/connect/account", authMiddleware, async (req, res) => {
  try {
    const user = req.user.username;
    const token = jwt.sign({ username: user }, JWT_SECRET, { expiresIn: "365d" });
